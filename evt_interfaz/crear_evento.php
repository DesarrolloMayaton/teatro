...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($titulo) > 200) $errores_php[] = "El título es demasiado largo (máximo 200 caracteres).";
    if (strlen($desc) > 5000) $errores_php[] = "La descripción es demasiado larga (máximo 5000 caracteres).";
    if (empty($_POST['funciones'])) $errores_php[] = "Debe agregar al menos una función.";
    if (empty($ini)) $errores_php[] = "Falta inicio de venta.";
    if ($tipo < 1 || $tipo > 2) $errores_php[] = "Tipo de escenario inválido.";
    ...
    if (!empty($_POST['funciones'])) {
        ...
        // Validación: cierre debe ser mayor que la última función
        if (!empty($fin)) {
            $ultimaFuncion = max(array_map('strtotime', $_POST['funciones']));
            if (strtotime($fin) <= $ultimaFuncion) {
                $errores_php[] = "La hora de cierre debe ser posterior a la última función.";
            }
        }
    }
    ...
    if (empty($errores_php)) {
        ...
        try {
            ...
            <style>
                ...
                .success-card { background: var(--bg-secondary); border-radius: 24px; padding: 40px; text-align: center; box-shadow: var(--shadow-xl); max-width: 380px; width: 90%; border: 1px solid var(--border-color); opacity: 0; transform: scale(0.9); animation: popIn 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
                .icon-circle { width: 80px; height: 80px; background: var(--success-bg); color: var(--success); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 40px; margin: 0 auto 20px; box-shadow: 0 0 0 8px rgba(50, 215, 75, 0.1); }
                .progress-track { height: 6px; background: var(--bg-tertiary); border-radius: 3px; margin-top: 30px; overflow: hidden; }
                .progress-fill { height: 100%; background: var(--success); width: 0; border-radius: 3px; transition: width 1.2s cubic-bezier(0.4, 0, 0.2, 1); }
...
<style>
    ...
    .form-label { color: var(--text-primary); font-weight: 600; margin-bottom: 8px; display: block; }
    .form-text { color: var(--text-muted) !important; font-size: 0.8rem; }
    ...
    .btn-primary:hover:not(:disabled) { background: var(--accent-blue-hover); transform: translateY(-2px); }
    ...
    .tooltip-error { color: var(--danger); font-size: 0.85em; margin-top: 5px; display: none; font-weight: 600; }
        ...
        <form id="fCreate" method="POST" enctype="multipart/form-data">
            <div class="row">
                <div class="col-12">
                    ...
                    <input type="text" id="tit" name="titulo" class="form-control" style="font-size: 1.1rem; font-weight: 600;" required placeholder="Nombre del evento">
                </div>

                <div class="col-12">
                    <div class="funciones-section">
                        <label class="form-label"><i class="bi bi-calendar-week me-2"></i>Gestión de Funciones</label>
                        ...
                        <div id="lista-funciones">
                            <p id="noFunc" style="color: var(--text-muted); margin: 0; width: 100%; text-align: center; font-style: italic; font-size: 0.9rem;">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" id="ini" name="inicio_venta" class="form-control" readonly required placeholder="Selecciona fecha y hora">
                ...
                <div class="col-md-6">
                    ...
                    <div class="cierre-container">
                        <input type="text" id="fin" name="cierre_venta" class="form-control" readonly required style="padding-right: 50px;">
                        <button type="button" class="cierre-lock-btn" id="lockBtn" title="Desbloquear edición manual">
                            <i class="bi bi-lock-fill"></i>
                        </button>
                    </div>
                    <div class="form-text"><i class="bi bi-info-circle"></i> Se calcula 2 horas después de la última función.</div>
                </div>
                
                <div class="col-12">
                    <label class="form-label">Descripción</label>
                    <textarea id="desc" name="descripcion" class="form-control" rows="4" required placeholder="Describe el evento..."></textarea>
            ...
            <hr>
            <button type="submit" id="bSub" class="btn btn-primary" style="width: 100%; padding: 16px; font-size: 1.1rem;" disabled>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    els.lockBtn.onclick = () => {
        ...
        if (cierreBloqueado) {
            ...
            // Recalcular cierre automático: 2 horas después de última función
            if (funcs.length) {
    ...
    els.add.onclick = () => {
        ...
        const now = new Date();
        // Margen de 1 minuto hacia el futuro para evitar "ya pasó hace 1 segundo"
        ...
        if (fpI.selectedDates.length) {
            ...
            // Margen: Función debe ser > Inicio + 1min
        ...
        if (!cierreBloqueado && fpE.selectedDates.length) {
            ...
            // Margen: Función debe ser < Cierre - 1min
    ...
    function upd() {
        ...
        if (!funcs.length) { 
            ...
            // Límite de inicio: eliminado para dar flexibilidad (se valida en val())
    ...
    function val() {
        ...
        if (!fpI.selectedDates.length) { 
            ...
            // Validar que inicio sea al menos 1 minuto antes de primera función
            // Nota: Se elimina restricción visual maxDate para evitar bloqueos
            if (funcs.length && new Date(fpI.selectedDates[0].getTime() + 60000) > funcs[0]) { 
        ...
        if (fpE.selectedDates.length && funcs.length && !cierreBloqueado) {
            const ultimaFuncion = funcs[funcs.length - 1];
            if (new Date(ultimaFuncion.getTime() + 60000) > fpE.selectedDates[0]) {
