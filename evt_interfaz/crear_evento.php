...
<style>
    ...
    body { background: linear-gradient(135deg, var(--bg-primary), #e2e8f0); color: var(--text-primary); font-family: 'Inter', sans-serif; padding: 30px; min-height: 100vh; }
...
<div class="main-wrapper">
    <a href="index.php" class="btn btn-secondary mb-4"><i class="bi bi-arrow-left me-2"></i>Volver</a>
    <div class="card">
        ...
        <form id="fCreate" action="procesar_evento.php" method="POST" enctype="multipart/form-data">
            <div class="row g-4">
                ...
                <div class="col-12"><label class="fw-bold">Descripción</label><textarea id="desc" name="descripcion" class="form-control" rows="3" required></textarea></div>
                <div class="col-md-7"><label class="fw-bold">Imagen</label><input type="file" id="img" name="imagen" class="form-control" accept="image/*" required></div>
                <div class="col-md-5">
...
function upd(){
    ...
    if(!funcs.length){ 
        ...
        fpE.set('minDate',now); fpE.clear(); // Limpia si no hay funciones
    }
    else{
        funcs.forEach((d,i)=>{
...
function val(){
    ...
    if(!document.getElementById('desc').value.trim()||!document.getElementById('img').files.length||!document.getElementById('tipo').value) ok=false;
