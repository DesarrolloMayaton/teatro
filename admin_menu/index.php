...
<style>
  :root{
    --aside:#2c3e50; --brand:#3498db; --bg:#f4f7f6; --paper:#fff; --bd:#e6ebf0; --ink:#2c3e50;
    --rad:14px; --shadow:0 10px 30px rgba(0,0,0,.08);
  }
  *{box-sizing:border-box}
  ...
  @media(max-width:900px){.row{grid-template-columns:repeat(6,1fr)}.col-6{grid-column:span 6}}
  label{font-weight:600;font-size:13px}
  input,select{width:100%;padding:10px 12px;border:1px solid var(--bd);border-radius:10px;background:#fff;outline:none}
  input:focus,select:focus{border-color:var(--brand);box-shadow:0 0 0 3px #3498db22}
  ...
  /* Tarjeta evento con miniatura */
  .evt-card{position:relative;background:#fff;border:1px solid var(--bd);border-radius:12px;box-shadow:var(--shadow);padding:14px 14px 14px 74px;min-height:68px}
  .evt-thumb{position:absolute;left:10px;top:10px;width:52px;height:52px;border-radius:8px;object-fit:cover;border:1px solid #e8ecf2;background:#f1f3f7}
...
<body>
  ...
  <main>
    <header>
      <div><strong id="title">Descuentos</strong> <span class="muted">· Admin</span></div>
      ...
      <section id="eventos">
        <div class="card">
          ...
          <table id="tabla-eventos">
            <thead>
              <tr>
                <th>Evento</th><th>Inicio venta</th><th>Cierre venta</th><th>Tipo</th><th>Aforo</th><th>Estado</th>
              </tr>
            </thead>
            <tbody></tbody>
          </table>
...
function buildImageCandidates(src){
  ...
  const baseDirs = ['', 'imagenes/', 'img/', 'uploads/','evt_interfaz/','evt_interfaz/imagenes/','evt_interfaz/uploads/'];
  return unique([
    ...(src.startsWith('/') ? [src] : []),
    ...baseDirs.map(d=> d+src),
    ...baseDirs.map(d=> '../'+d+src),
    ...baseDirs.map(d=> '../../'+d+src)
  ]);
}
...
// ===== API Promos (todo via POST + action) =====
const API = 'promos_api.php';

async function apiListPromos(){
  const r = await fetch(API + '?action=list', { cache:'no-store' });
  const j = await r.json(); if(!j.ok) throw new Error(j.error||'Error API'); return j.items||[];
}
async function apiCreatePromo(p){
  const r = await fetch(API + '?action=create', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(p) });
  const j = await r.json(); if(!j.ok) throw new Error(j.error||'No se pudo crear'); return j.id_promocion;
}
async function apiUpdatePromo(id, p){
  const r = await fetch(API + '?action=update&id='+encodeURIComponent(id), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(p) });
  const j = await r.json(); if(!j.ok) throw new Error(j.error||'No se pudo actualizar');
}
async function apiDeletePromo(id){
  const r = await fetch(API + '?action=delete&id='+encodeURIComponent(id), { method:'POST' });
  const j = await r.json(); if(!j.ok) throw new Error(j.error||'No se pudo eliminar');
}
function exportCSV(){ window.open(API+'?action=export','_blank'); }
...
(async function init(){
  ...
  // Delegación en TBODY (editar/borrar)
  document.querySelector('#tabla-promos tbody').addEventListener('click', onPromosTBodyClick, { passive:true });
...
async function onPromosTBodyClick(ev){
  ...
  btn.disabled = true; const prevOpacity = btn.style.opacity; btn.style.opacity = .6;

  try {
    if (act === 'del') {
      if (confirm('¿Eliminar esta promoción de la base de datos?')) {
      ...
      // Precargar formulario para actualizar
      $('#ev').value     = p.evId ?? '';
      $('#tipo').value   = p.tipo;
      ...
      go('descuentos');
      previewPromo();
      document.querySelector('#form-descuento')?.scrollIntoView({ behavior:'smooth', block:'start' });
    }
  } catch (err) {
    alert('Operación fallida: ' + (err?.message || err));
  } finally {
    btn.disabled = false; btn.style.opacity = prevOpacity || '';
  }
}
...
async function renderEventosCards(){
  ...
  if(!state.eventos.length){ grid.innerHTML = `<div class="muted">No se encontraron eventos.</div>`; return; }
  for(const e of state.eventos){
    ...
    if(url){
      ...
      img.className = 'evt-thumb'; img.alt = 'thumb'; img.src = url;
      ph.replaceWith(img);
    }else{
      ph.textContent = 'Sin\nfoto';
    }
